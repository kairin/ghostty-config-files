...
test_validate_node_missing() {
    ...
    if validate_node_installation; then
        echo "❌ FAIL: Should return false when node missing"
        return 1
    else
        echo "✅ PASS: Correctly detects missing node"
        return 0
    fi
}
...
test_validate_node_present() {
    ...
    if validate_node_installation; then
        echo "✅ PASS: Correctly detects present node"
        rm -rf "$mock_dir"
        return 0
    else
        echo "❌ FAIL: Should return true when node present"
        rm -rf "$mock_dir"
        return 1
    fi
}
...
4. **Iterate**: Implement → Test → Validate → Commit → Repeat
