# ~/.config/fish/functions/font-picker.fish
# Function name MUST equal the file basename (`font-picker`) so fish autoloads it.
...
function font-picker --description 'Pick a Nerd Font for Ghostty via zenity'
    ...
    # zenity cancel / Esc / empty selection -> exit cleanly, do not touch config.
    if test -z "$selected"
    ...
    # NOTE: this replaces EVERY ^font-family line; the config keeps exactly one.
    # Delimiter '|' is safe: Nerd Font names contain no '|'. Write UNQUOTED to
    # match the config's default form (Ghostty accepts unquoted multi-word names).
    ...
    # Reload Ghostty now. SIGUSR2 is Ghostty's documented live-reload signal on
    # Linux/GTK (verified: binary logs "received SIGUSR2, reloading configuration").
