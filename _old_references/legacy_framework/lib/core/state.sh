...
load_state() {
    ...
    done < <(jq -r '.completed_tasks[]' "$STATE_FILE" 2>/dev/null || true)
...
update_task_duration() {
    ...
    # Ensure duration is a valid number (strip any non-numeric chars except decimal point)
    duration="${duration//[^0-9.]/}"
    duration="${duration:-0}"

    # Update task duration using jq with error handling
    if ! jq --arg task_id "$task_id" --argjson duration "$duration" \
        '.performance.task_durations[$task_id] = $duration' \
        "$STATE_FILE" > "${STATE_FILE}.tmp" 2>/dev/null; then
        log "WARNING" "Failed to update task duration for $task_id"
        return 1
    fi
