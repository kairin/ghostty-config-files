...
scan_apt_packages() {
    log "INFO" "Scanning APT packages..." >&2
    ...
    local count
    count=$(wc -l < "$tmp_data")
    log "INFO" "  Found $count APT packages" >&2
...
scan_snap_packages() {
    log "INFO" "Scanning Snap packages..." >&2
    local packages_json="[]"
    
    if ! command -v snap &> /dev/null; then
        log "INFO" "  Snapd not installed, skipping snap scan" >&2
        echo "$packages_json"
        return 0
    fi
    ...
    log "INFO" "  Found $count Snap packages ($disabled_count disabled)" >&2
...
scan_desktop_files() {
    log "INFO" "Scanning desktop files..." >&2
    ...
    local count
    count=$(wc -l < "$tmp_data" 2>/dev/null || echo "0")
    log "INFO" "  Found $count desktop applications" >&2
