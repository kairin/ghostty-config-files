...
detect_duplicates() {
    ...
    log "INFO" "Detecting duplicate applications..." >&2
    ...
    for ((i=0; i<snap_count; i++)); do
        ...
        for apt_name in $apt_names; do
            ...
            if [ "$apt_exists" = "true" ]; then
                ...
                log "INFO" "  Duplicate: $snap_name (snap + apt)" >&2
                break
            fi
        done
    done

    log "INFO" "  Found $count duplicate applications" >&2
...
detect_disabled_snaps() {
    ...
    log "INFO" "Detecting disabled snap packages..." >&2
    ...
    log "INFO" "  Found $count disabled snaps (${total_size}MB total)" >&2
    echo "$disabled_json"
}
...
detect_browsers() {
    ...
    log "INFO" "Detecting installed browsers..." >&2
    ...
    log "INFO" "  Found $count browsers installed" >&2

    # Add recommendation if >3 browsers
    if [ "$count" -gt 3 ]; then
        log "WARNING" "  Multiple browsers detected - recommend keeping 1-2" >&2
    fi
