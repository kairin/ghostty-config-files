...
main() {
    log "INFO" "Starting Node.js FNM installation (5 steps)..."
    ...
    for step in "${steps[@]}"; do
        ...
        if ! "${STEPS_DIR}/${step}"; then
            log "ERROR" "Node.js FNM installation failed at step ${step_num}/${total_steps}: ${step_name}"
    ...
    log "SUCCESS" "Node.js FNM installation complete (${total_steps}/${total_steps} steps)"
    return 0
}
